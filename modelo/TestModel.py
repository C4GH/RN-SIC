...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Convert label from probability list to class index
        label = torch.tensor(item[1]).argmax().item()
...
def collate_fn(batch):
    ...
    labels_tensor = torch.tensor(labels_list, dtype=torch.long)  # Make sure labels are long integers
