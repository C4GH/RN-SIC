...
with open('../texto/vectors.txt', 'rt') as fi:
...
vocab_npa = np.arra(vocab, dtype='<U10') # ajuste del tipo de datos.
ems_npa = np.array(embeddings)
...
sep_emd_npa = np.mean(ems_npa, axis=0, keepdims=True) # Embedding promedio para '<sep>'

# Añadir embeddings para tokens especiales
ems_npa =np.vstack((pad_emb_npa, sep_emd_npa, ems_npa))
...
np.save('./salda/vocab_npa.npy', vocab_npa)
np.save('./salda/embs_npa.npy', embs_npa)
